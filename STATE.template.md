...
2. **Claim.** Take one unit from §4 Open edges. Add a row to §3 In flight with
...
4. **Receipt + record.** Before you call it done, produce a receipt: a green
   ...
   from §3 to §6 with that receipt.

Conflict-avoidance = the claim ledger (§3) + branch isolation.
